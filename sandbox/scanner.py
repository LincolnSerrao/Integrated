...
from .steg_scanner import scan_image
...
def _supports_pe_ml(file_path: Path) -> bool:
    ext = file_path.suffix.lower()
    if ext in PE_ML_EXTENSIONS or ext in WINDOWS_INSTALLER_EXTENSIONS:
        return True
...
def write_scan_event(payload: dict[str, Any]) -> dict[str, Any]:
    event = dict(payload)
    event.setdefault("ts", datetime.now(timezone.utc).isoformat(timespec="seconds").replace("+00:00", "Z"))
    with log_lock:
...
def _ml_scan(
    ...
    try:
        from Backend.app.model_def import ZeroDayDetector
    except Exception as exc:
...
def scan_file(
    ...
    try:
        ...
        if ext in {".png", ".jpg", ".jpeg"}:
            ...
            if not np.isfinite(static_prob) or not np.isfinite(fused_risk):
                raise ScannerStageError("non_finite_score", "Model produced a non-finite risk score")
        else:
            result = _heuristic_scan(target)
            result["scanner_stage"] = "unsupported_type"
            result["scanner_warning"] = "PE ML scanning is only applied to Windows executable formats"
    except Exception as exc:
        result = _heuristic_scan(target)
