...
function formatSafetyScore(value) {
  ...
  const formatted = formatPreciseNumber((1 - value) * 100, 14);
...
export default function ResultPage({ overallResult }) {
  ...
  const riskDisplay = !hasFile ? '--' : formatRiskPercent(risk);
  const scoreDisplay = !hasFile ? '--' : (risk === null ? '--' : `${formatSafetyScore(risk)}/100`);
  ...
  const tags = useMemo(() => {
    ...
    const engineTag = `Engine: ${formatEngineLabel(scan.engine)}`;
    const riskTag = risk === null ? 'Risk: N/A' : `Risk: ${formatRiskPercent(risk)}`;
    ...
    <section className="page">
      ...
      <div className="result-grid">
        <article className="card result-main">
          <h3>Risk</h3>
          <p className="score">{riskDisplay}</p>
          <p className={resultClass(resultText)}>Result: {resultText}</p>
          <p className="muted-text">Exact fused risk reported by the backend.</p>
          <p className="muted-text">Safety score: {scoreDisplay}</p>
        </article>
